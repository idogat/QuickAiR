...
function renderExecute() {
  ...
  var addSectionHTML =
    '<div class="exec-add-section">' +
      ...
      '</div>' +
      '<div class="exec-add-body" id="exec-add-body">' +
  ...
  if (allHosts.length === 0) {
    ...
    collectedHosts.forEach(function(h) {
      ...
      rows += '<tr id="exec-row-' + cssId(h) + '">' +
        ...
        '<td><span class="' + cs.cls + '">' + (cs.status === 'complete' || cs.status === 'partial' ? 'Yes' : 'No') + '</span></td>' +
...
function execSelectAllMem(headerCb) {
  var hosts = Object.keys(_bulkSel);
  hosts.forEach(function(h) {
    _bulkSel[h].mem = headerCb.checked;
    var cb = el('exc-mem-' + cssId(h));
...
function execSelectAllDisk(headerCb) {
  var hosts = Object.keys(_bulkSel);
  hosts.forEach(function(h) {
    _bulkSel[h].disk = headerCb.checked;
    var cb = el('exc-dsk-' + cssId(h));
    if (cb) cb.checked = headerCb.checked;
  });
  _execUpdateSelCounter();
...
function _execUpdateSelCounter() {
  ...
  var memAll = el('exc-mem-all');
  if (memAll) {
    memAll.checked       = total > 0 && memJobs === total;
    memAll.indeterminate = memJobs > 0 && memJobs < total;
  }
  var dskAll = el('exc-dsk-all');
  if (dskAll) {
    dskAll.checked       = total > 0 && diskJobs === total;
    dskAll.indeterminate = diskJobs > 0 && diskJobs < total;
  }
}
