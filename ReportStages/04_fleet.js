...
function renderFleet() {
  ...
  const COLS = '36px 180px 200px 60px 50px 110px 80px 100px 100px 80px 120px';
  const headers = [
    { key:'_remove',            label:''                 },
  ...
  function buildRows() {
    ...
    if (sortState.tab === 'fleet' && sortState.column && sortState.dir !== 'none') {
      const numericFleetCols = new Set(['nic_count','proc_count','active_count','unique_ips','dns_count']);
  ...
  mp.innerHTML = `<div class="mp-wrap">
    ...
    <div class="mp-body">
      <div class="mp-grid">
        ...
        <span class="k">sha256</span>                  <span class="v">${esc(m.sha256||'')}</span>
      </div>
    </div>
  </div>
  ${adapterTableHTML}`;
