...
const UCOLS = '160px 100px 130px 110px 130px 110px 70px 100px';
...
function fmtTs(s)      { if (!s) return '&#8212;'; return esc(s.slice(0, 19).replace('T', ' ')); }
...
function usersSortRows(rows) {
  ...
  return rows.slice().sort((a, b) => {
    ...
    switch (col) {
      ...
      case 'machines':   return (a.machines.length - b.machines.length) * dir;
      case 'confidence': {
  ...
  panel.innerHTML = (showDcBanner ? `
    ...
    <div class="tbl-wrap">
      <div class="tbl-header" style="grid-template-columns:${UCOLS}" id="users-header">
        ...
        <div class="th sortable-th" onclick="sortTable('users','machines',true)">Machines <span class="sort-arrow" id="sort-users-machines">&#8645;</span></div>
...
function renderUserRow(u) {
  ...
  const dcDate   = u.domainInfo ? fmtDate(u.domainInfo.WhenCreated) : (
    u.AccountType === 'Local'
  ...
  let firstLoginCell;
  if (fl) {
    firstLoginCell = fmtDate(fl.UTC) + ' ' + confBadge(fl.Confidence);
  } else {
  ...
  return `
    ...
    <div class="td dim" title="${esc(USER_TIPS.LastLogon)}">${fmtDate(lastLogon)}</div>
    <div class="td dim" style="text-align:center">${u.machines.length || '<span style="color:var(--muted)">&#8212;</span>'}</div>
...
function buildUserExpand(u) {
    ...
    html += `
      ...
      <div class="kv-grid" style="margin-bottom:12px">
        <span class="k" title="${esc(USER_TIPS.WhenCreated)}">Created</span>
          <span class="v">${fmtTs(di.WhenCreated)}</span>
        <span class="k" title="${esc(USER_TIPS.LastLogonAD)}">Last Logon (AD)</span>
          <span class="v">${fmtTs(di.LastLogonAD)}</span>
        <span class="k">Password Last Set</span>
          <span class="v">${fmtTs(di.PasswordLastSet)}</span>
  ...
  if (u.machines.length > 0) {
    ...
    u.machines.forEach(m => {
      ...
      const tip = 'Profile folder:\n  ' + (pf.Value || 'N/A') +
                  '\nNTUSER.DAT:\n  ' + (nt.Value || 'N/A') +
                  '\nProfileList key\n  (' + (rk.KeyPath || 'HKLM\\...\\ProfileList\\<SID>') + '):\n  ' + (rk.Value || 'N/A') +
                  '\nConfidence: ' + (fl.Confidence || '?');
      const lastTip = 'Source: ' + (m.LastLogonSource || 'ProfileList LastUseTime') + '\nupdated on each logoff';
      ...
      html += `<tr>
        ...
        <td title="${esc(tip)}">${fl.UTC ? esc(fl.UTC.slice(0,10)) : '<span style="color:var(--muted)">&#8212;</span>'} ${confBadge(fl.Confidence)}</td>
        <td title="${esc(USER_TIPS.Confidence)}">${confBadge(fl.Confidence)}</td>
        <td title="${esc(lastTip)}">${m.LastLogon ? esc(m.LastLogon.slice(0,10)) : '<span style="color:var(--muted)">&#8212;</span>'}</td>
