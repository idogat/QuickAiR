...
function usersSortRows(rows) {
  ...
  return rows.slice().sort((a, b) => {
    let av, bv;
    switch (col) {
      case 'username':   av = str(a.username || a.SID); bv = str(b.username || b.SID); break;
      case 'acctType':   av = str(a.appearances.length > 0 ? a.appearances[0].AccountType : '');
                         bv = str(b.appearances.length > 0 ? b.appearances[0].AccountType : ''); break;
      case 'dcCreated':  av = a.domainAccount ? (a.domainAccount.WhenCreated || '') : '';
                         bv = b.domainAccount ? (b.domainAccount.WhenCreated || '') : ''; break;
      case 'hosts':      return (a.appearances.length - b.appearances.length) * dir;
      case 'firstSeen':  av = usersGetFirstSeen(a) || ''; bv = usersGetFirstSeen(b) || ''; break;
      case 'lastSeen':   av = usersGetLastSeen(a) || ''; bv = usersGetLastSeen(b) || ''; break;
      case 'active':     av = a.sessions.length > 0 ? 'yes' : ''; bv = b.sessions.length > 0 ? 'yes' : ''; break;
      case 'domainAcct': av = a.domainAccount ? 'yes' : ''; bv = b.domainAccount ? 'yes' : ''; break;
      case 'confidence': {
  ...
  container.innerHTML = `
    <div class="tbl-wrap">
      <div class="tbl-header" style="grid-template-columns:${UCOLS}" id="users-header">
        ...
        <div class="th sortable-th" onclick="sortTable('users','acctType',false)">Acct Type <span class="sort-arrow" id="sort-users-acctType">⇅</span></div>
        ...
        <div class="th sortable-th" onclick="sortTable('users','hosts',true)">Hosts <span class="sort-arrow" id="sort-users-hosts">⇅</span></div>
        <div class="th sortable-th" title="${esc(USER_TIPS.FirstLogon)}" onclick="sortTable('users','firstSeen',false)">First Seen <span class="sort-arrow" id="sort-users-firstSeen">⇅</span></div>
        <div class="th sortable-th" title="${esc(USER_TIPS.LastUseTime)}" onclick="sortTable('users','lastSeen',false)">Last Seen <span class="sort-arrow" id="sort-users-lastSeen">⇅</span></div>
        <div class="th sortable-th" onclick="sortTable('users','active',false)">Active <span class="sort-arrow" id="sort-users-active">⇅</span></div>
        <div class="th sortable-th" onclick="sortTable('users','domainAcct',false)">Domain Acct <span class="sort-arrow" id="sort-users-domainAcct">⇅</span></div>
...
function renderUserRow(u) {
  const hasMismatch   = u.appearances.some(a => a.TimestampMismatch);
  ...
  let rowClass = '';
  if (hasMismatch && bestConf === '?') rowClass = 'color:var(--amber)';
  ...
  const firstSeen = usersGetFirstSeen(u);
  const lastSeen  = usersGetLastSeen(u);
  ...
  (function() {
    ...
    if (profiles.length) {
      ...
      profiles.forEach(p => {
        ...
        const tamperStyle = fl.TimestampMismatch ? 'border-left:3px solid var(--amber)' : '';
